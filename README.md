...
**Auth setup — both providers must be enabled in the Supabase dashboard:**

- **Email** (Authentication → Sign In / Providers → Email): required for member signups and the admin panel's Add User. Emails are auto-confirmed by a DB trigger, so no SMTP is needed.
- **Google** — guest access:
